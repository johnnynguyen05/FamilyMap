...
public class ServerTest {
    ...
    public void testValidNewRegister(TestInfo testInfo) {
        ...
        try {
            ...
            assertNotEquals(EMPTY_STRING, result.getPersonID(), "personID was empty string, expected non-empty string containing the personID of the user's generated Service.Person object");
     ...
     */
    @Test
    @DisplayName("Service.Person Valid Test")
     ...
     */
    @Test
    @DisplayName("Service.Person Wrong User Test")
     ...
     * Required API calls:
     * Register
     * (Partial) Service.Person/[personID]
     */
    @Test
    @DisplayName("Service.Person Bad Auth Token Test")
    ...
    public void testValidFillRelationships(TestInfo testInfo) {
        ...
        try {
            ...
            //We are calling the get all persons api for the user sheila (we are using the authtoken variable from registerResult on the previous line)
            PersonsResult personsResult = proxy.persons(host, port, registerResult.getAuthtoken());
            //Here we are getting the Service.Person information for the user sheila
    ...
    public void testRealisticBirthEvents(TestInfo testInfo) {
        ...
        try {
            ...
            //Here we are getting the Service.Person information for the user sheila
            Person userPerson = personsResult.getPerson(registerResult.getPersonID());
            //If person is null then a Service.Person Object was not created for the user and inserted into the database
            assertNotNull(userPerson, "User's Service.Person not included in passoffresult");
    ...
    public void testRealisticDeathEvents(TestInfo testInfo) {
        ...
        try {
            ...
            //Here we are getting the Service.Person information for the user sheila
            Person userPerson = personsResult.getPerson(registerResult.getPersonID());
            //If person1 is null then a Service.Person Object was not created for the user and inserted into the database
            assertNotNull(userPerson, "User's Service.Person not included in passoffresult");
            //Here we are getting the Service.Person information for sheila's father
            Person userFather = personsResult.getPerson(userPerson.getFatherID());
            //Here we are getting the Service.Person information for sheila's mother
            Person userMother = personsResult.getPerson(userPerson.getMotherID());
            //If person2 is null then a Service.Person Object was not created for the user's father and inserted into the database
            assertNotNull(userFather, "User's Father's Service.Person not included in passoffresult");
            //If person3 is null then a Service.Person Object was not created for the user's mother and inserted into the database
            assertNotNull(userMother, "User's Mother's Service.Person not included in passoffresult");
    ...
    /**
     * First checks to make sure the test that called this function received an
     * HTTP_BAD_REQUEST header. Then makes sure all the Service.Person List is
     * empty. Ends by verifying that an error was reported.
     *
     * @param personsResult A PersonsResult holding an empty Service.Person List and an error message
     */
     ...
     * @param person          Service.Person we are looking at
    ...
    private void checkParentsMarriage(EventsResult eventsResult, PersonsResult personsResult, Person person, String relationship, int generationsLeft) {
        ...
        assertNotNull(personFather, relationship + "'s Father's Service.Person not included in passoffresult");
        ...
        assertNotNull(personMother, relationship + "'s Mother's Service.Person not included in passoffresult");
